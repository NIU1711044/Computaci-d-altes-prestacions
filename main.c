...
//-----------FREE INPUT------------
// Allibera la memòria que s'ha reservat per cada imatge/vector d'entrada
...
//-----------PRINTRECOGNIZED------------
// Busca la neurona de sortida amb activació més gran --> és la predicció final
// del model.
void printRecognized(int p, layer Output) {
    ...
    if (imax == Validation[p]) // Validation[p] conté el valor correcte
        total++;
...
void train_neural_net() {
    ...
    int ranpat[num_training_patterns]; // Vector on s'emmagatzema els índexs dels patrons d'entrenament, es barrejaran aleatòriament a cada epoch. 
...
// No es pot paral·lelitzar, hi ha dependències
